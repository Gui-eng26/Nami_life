...
│       └── 20260811000000_data_nascimento_users.sql      → coluna users.data_nascimento (date, nullable) (MH-072 Parte A, v30), aplicada manualmente. Cascata de exclusão de conta já coberta por DELETE FROM users em delete_user_account — nenhuma alteração necessária ali (coluna vive na própria tabela apagada)
...
### Requisito confirmado para o escopo da Parte B

Guilherme pediu (12/08, fim da sessão): destacar `**DIA**`, `**MÊS**` e `**ANO**` em
maiúsculas e negrito nas perguntas de cada etapa da coleta. Motivo: no WhatsApp o negrito
é o único recurso tipográfico disponível, e para o público-alvo idoso ele separa "o que
estão me pedindo agora" do resto da frase — hoje a Nami usa negrito de forma inconsistente
entre turnos. Deve ser regra fixa no prompt, não sugestão — deixar a cargo da geração
livre repetiria o padrão que causou três defeitos nesta sessão. **Decisão de Guilherme
(12/08): entra como escopo do briefing da Parte B**, não como item novo — não requer
registro próprio no backlog; o briefing de MH-072 Parte B deve incluir essa regra.

### Próximo

MH-072 Parte B — reestruturação do recepcionista, separando classificação de geração.
Resolve BUG-030 (o `pareceNome()` é uma lista de exclusão de palavras, o antipadrão do
princípio 14) e MH-074 (pergunta funcional não exige nome nem LGPD). O `querCadastrar` em
`src/estadoPosOnboarding.js` é alvo declarado da mesma parte — é outra lista de palavras.
Escopo confirmado também inclui o negrito de `**DIA**`/`**MÊS**`/`**ANO**` acima.
...
Restante para o beta: MH-040 (mensagens fracionadas), MH-073 (gotas), MH-009 (dashboard).
...
mensagem de chat, em bloco separado. Ver o parágrafo de migrations acima ("Migrations NÃO são
aplicadas por deploy"), corrigido nesta sessão pelo mesmo motivo.
...
39. **Quando o código apaga ou invalida um dado, o prompt de geração precisa ser
    informado explicitamente disso — extensão do princípio 38 (v30, MH-072 A.2 item
    2).** A LLM lê a MENSAGEM DO TURNO ATUAL (não tem acesso ao histórico completo em
    `data_nascimento.js`), e se essa mensagem contém um valor que o sistema acabou de
    descartar, ela confirma esse valor por conta própria — mesmo com a regra "nunca
    proponha fora de `camposPreenchidos`" já em vigor (princípio 38), porque o valor
    está ali na própria mensagem do usuário, não "inventado" do nada. Todo caminho que
    zera um campo (`nasc_negacao` Caso B) declara isso ao gerador via `campoZerado`,
    com instrução de ignorar qualquer menção a esse campo na mensagem atual.
